...
接收发送数据使用二进制数据流的方式，所以可以非常方便的配合Protobuf使用。
...
## Connection连接
每个连接建立时，触发连接事件。封装了接收和发送的方法，接收到消息时使用channel发送到socketservice，所以在游戏逻辑中是同步的。
在读取消息时，加入超时检查来实现心跳检查，如果在设置的时间内没有接收到消息，则判断为心跳丢失触发断线事件。
